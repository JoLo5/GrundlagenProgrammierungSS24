...
# A4 ----
mehraugen <- factor(c(as.character(augenfarbe), "nussbraun"))
...
personenMitFarbe <- function(namen, farben) {
  farben_tabelle <- table(farben)
  haeufigste_farbe <- names(farben_tabelle)[which.max(farben_tabelle)]
  namen[farben == haeufigste_farbe]
}
...
mmmmEinkommen <- t(sapply(split(einkommen, augenfarbe), function(x) {
...
# tb_start
mmmmEinkommenKurz <- t(sapply(split(einkommen, augenfarbe), function(x)
  c(min = min(x), median = median(x), mean = mean(x), max = max(x))
))
