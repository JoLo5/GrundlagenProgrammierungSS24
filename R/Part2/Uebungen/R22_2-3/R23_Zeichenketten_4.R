...
datum_mit_minus <- function(daten) {
  sapply(daten, function(d) {
    if (grepl("-", d)) {
      return(d)  # Bereits formatiertes Datum zurückgeben
    } else {
      # Zerlegen in Jahr, Monat, Tag
      jahr <- substr(d, 1, 4)
      monat <- substr(d, 5, 6)
      tag <- substr(d, 7, 8)
      return(paste(jahr, monat, tag, sep = "-"))
    }
  })
}
...
anzahlWorte <- length(strsplit(Rtext, "\\s+")[[1]])
...
textSätze <- unlist(strsplit(Rtext, "(?<=[.!?])\\s+", perl = TRUE))



# B2 ----
sentence <- "Dies soll eine Schlagzeile in einer Zeitung werden."
headline <- gsub("(^|\\s)([a-z])", "\\1\\U\\2", sentence, perl = TRUE)
"Dies soll eine Schlagzeile in einer Zeitung werden."
