...
anzahlNA <- function(x) sum(is.na(x))
positionNA <- function(x) which(is.na(x))
...
nurKomplett <- function(x) x[complete.cases(x), ]
...
med_mit <- function(x) c(median(x, na.rm = TRUE), mean(x, na.rm = TRUE))
...
# t4_start
NAproSpalte <- colSums(is.na(airquality))
